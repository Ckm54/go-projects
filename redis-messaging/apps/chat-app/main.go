...
// associate username with websocket client session objects
...
func ChatAppMain() {
	...
	// subscribe to redis pub/sub channel, listen for incoming messages and broadcast to all connected users excluding the sender
	go func() {
		...
		for message := range messages {
			...
			for user, peer := range Users {
				if from != user {
					...
					err := peer.WriteMessage(websocket.TextMessage, []byte(msg))
					if err != nil {
	...
	// handles websocket connection for users joining the chat
	...
	for {
		...
		if err != nil {
			...
			if ok {
				...
				err := c.Close()
				if err != nil {
					fmt.Println("close connenection error", err)
				}
				delete(Users, user)
		...
		err = client.Publish(context.Background(), channelName, user+":"+string(message)).Err()
		if err != nil {
