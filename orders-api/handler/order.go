...
import (
	...
	"github.com/ckm54/orders-api/model"
	"github.com/ckm54/orders-api/repository/order"
...
func (h *Order) UpdateByID(w http.ResponseWriter, r *http.Request) {
	...
	orderToUpdate, err := h.Repo.FindByID(r.Context(), orderID)
	if errors.Is(err, order.ErrorNotExist) {
		w.WriteHeader(http.StatusNotFound)
		return
	} else if err != nil {
	...
	switch body.Status {
	case shippedStatus:
		if orderToUpdate.ShippedAt != nil {
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		orderToUpdate.ShippedAt = &now
	case completedStatus:
		if orderToUpdate.CompletedAt != nil || orderToUpdate.ShippedAt == nil {
			w.WriteHeader(http.StatusBadRequest)
			return
		}
	...
	err = h.Repo.Update(r.Context(), orderToUpdate)
	if err != nil {
	...
	if err := json.NewEncoder(w).Encode(orderToUpdate); err != nil {
